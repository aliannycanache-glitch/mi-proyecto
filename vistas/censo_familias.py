...
def vista_familia(pagina: ft.Page):
    ...
    def guardar_excel_seleccionado(e: ft.FilePickerResultEvent):
        if e.path:
            ...
            dlg = ft.AlertDialog(
                ...
                actions=[
                    ft.TextButton(
                        "Aceptar", on_click=lambda ev: e.page.close(dlg)
                    )
                ],
            )
            e.page.open(dlg)
    ...
    file_picker = ft.FilePicker(on_result=exportar_pdf_handler)
    ...
    def filtrar_datos():
        ...
        campo_busqueda = (
            tipo_busqueda.current.value if tipo_busqueda.current else "Nombre"
        )
    ...
    def actualizar_tarjetas():
        ...
        total_menores = sum(
            1 for fecha in edades if _edad_en_meses(fecha) is not None and _edad_en_meses(fecha) < 216
        )
        total_mayores = sum(
            1 for fecha in edades if _edad_en_meses(fecha) is not None and _edad_en_meses(fecha) >= 216
        )

        tarjeta_familias.current.content.controls[1].value = str(total_familias)
        tarjeta_personas.current.content.controls[1].value = str(total_personas)
    ...
    def actualizar_tabla():
        ...
        for f in visibles:
            ...
            tabla.current.rows.append(
                ft.DataRow(
                    cells=[
                        ft.DataCell(ft.Container(ft.Text(f"{f.nombres_jefe} {f.apellidos_jefe}"), width=200)),
                        ft.DataCell(ft.Container(ft.Text(f"{f.tipo_id}-{f.cedula_jefe}"), width=140)),
                        ft.DataCell(ft.Container(ft.Text(f.telefono_jefe or ""), width=140)),
                        ft.DataCell(ft.Container(ft.Text(direccion_completa), width=280)),
                        ft.DataCell(ft.Container(ft.Text(f"{len(f.miembros)} personas"), width=120)),
                        ft.DataCell(
                            ft.Container(
                                ft.Row(
                                    [
                                        ft.Container(
                                            content=ft.Row(
                                                [
                                                    ...
                                                    ft.Text("Editar", color=COLOR_VERDE),
                                                ],
                                                spacing=4,
                                            ),
    ...
    tarjeta_familias.current = ft.Container(
        content=ft.Column(
            [
                ft.Text("Familias Registradas", size=14, color=COLOR_GRIS, weight="bold"),
    ...
    tarjeta_personas.current = ft.Container(
        content=ft.Column(
            [
                ft.Text("Personas en Censo", size=14, color=COLOR_GRIS, weight="bold"),
                ft.Text("0", size=22, weight="bold", color=COLOR_NEGRO),
            ]
        ),
        bgcolor=COLOR_BLANCO,
    ...
    encabezado = ft.Row(
        [
            ft.Text("Censo de Familias", size=26, weight="bold", color=COLOR_NEGRO),
            ft.Row(
                [
                    ft.OutlinedButton(
                        "Cargar Excel",
...
def vista_registro_familia(pagina=None, edit_id=None):
    ...
    edad_jefe = ft.TextField(label="Edad", width=90, disabled=True)
    fecha_nacimiento_jefe = ft.TextField(label="Fecha de Nacimiento", width=220, read_only=True)
    ...
    selector_bonos = ft.Container(
        content=ft.Column(
            [
                ft.Text("Bonos o beneficios (puede seleccionar varios)", weight="bold", color=COLOR_NEGRO),
    ...
    calle_jefe = ft.Dropdown(label="Calle", width=280, options=opciones_calles)
    numero_casa_jefe = ft.TextField(label="N° de Casa", width=160)

    nombres_jefe = ft.TextField(label="Nombres", width=280)
    apellidos_jefe = ft.TextField(label="Apellidos", width=280)

    # Campos para la Carga Familiar con anchos optimizados
    edad_miembro = ft.TextField(label="Edad", width=80, disabled=True)
    fecha_nacimiento_miembro = ft.TextField(label="F. Nacimiento", width=140, read_only=True)
    ...
    nombres_miembro = ft.TextField(label="Nombres", width=180)
    apellidos_miembro = ft.TextField(label="Apellidos", width=180)
    ...
    encabezados_tabla = ft.Container(
        content=ft.Row(
            [
                ...
                ft.Text("Tipo ID", width=90, weight="bold"),
                ft.Text("Cédula", width=140, weight="bold"),
                ft.Text("F. Nacimiento", width=130, weight="bold"),
                ft.Text("Edad", width=80, weight="bold"),
                ft.Text("Parentesco", width=130, weight="bold"),
                ft.Text("Acciones", width=100, weight="bold"),
            ],
            spacing=10,
        ),
    ...
    def añadir_miembro(e: ft.ControlEvent):
        if not nombres_miembro.value.strip() or not apellidos_miembro.value.strip():
            e.page.open(ft.SnackBar(ft.Text("Por favor completa los campos obligatorios")))
            return

        cedula_valor = (
            cedula_miembro.value.strip() if cedula_miembro.value.strip() else "No posee"
        )
        ...
        def editar_miembro(ev, miembro):
            ...
            cedula_miembro.value = "" if miembro["cedula"] == "No posee" else miembro["cedula"]
            fecha_nacimiento_miembro.value = miembro["fecha_nacimiento"]
            edad_miembro.value = miembro["edad"]
            parentesco_miembro.current.value = miembro["parentesco"] if miembro["parentesco"] != "—" else None
        ...
        fila = ft.Container(
            content=ft.Row(
                [
                    ...
                    ft.Text(miembro_data["tipo"], width=90),
                    ft.Text(miembro_data["cedula"], width=140),
                    ft.Text(miembro_data["fecha_nacimiento"], width=130),
                    ft.Text(miembro_data["edad"], width=80),
                    ft.Text(miembro_data["parentesco"], width=130),
                    ft.Row([
                        ft.IconButton(
                            icon=ft.Icons.EDIT,
                            icon_color=COLOR_VERDE,
                            tooltip="Editar",
                            on_click=lambda ev, m=miembro_data: editar_miembro(ev, m),
                        ),
                        ft.IconButton(
                            icon=ft.Icons.DELETE,
                            icon_color=COLOR_ROJO,
                            tooltip="Eliminar",
                            on_click=lambda ev, m=miembro_data: eliminar_miembro(ev, m),
                        ),
                    ], width=100),
                ],
                spacing=10,
        ...
        for miembro in familia_edicion.miembros:
            ...
            fila_ref[0] = ft.Container(
                content=ft.Row([
                    ft.Text(miembro_data["nombres"], width=160),
                    ft.Text(miembro_data["apellidos"], width=160),
                    ft.Text(miembro_data["tipo"], width=95),
                    ft.Text(miembro_data["cedula"], width=140),
                    ft.Text(miembro_data["fecha_nacimiento"], width=130),
                    ft.Text(miembro_data["edad"], width=80),
                    ft.Text(miembro_data["parentesco"], width=132),
                    ft.Row([
                ...
                bgcolor="#83A8E8C7",
                border_radius=6,
                border=ft.border.all(1, "#76E1E4A9"),
            )
    ...
    boton_cancelar = ft.OutlinedButton("Cancelar", on_click=lambda e: e.page.go("/familia"), height=45)

    # Organización responsiva y estéticamente limpia en tarjetas
    tarjeta_jefe = ft.Container(
        content=ft.Column([
            ft.Text("Datos del Jefe de Familia", size=18, weight="bold", color=COLOR_NEGRO),
        ...
        bgcolor="#7692DEA7",
        border_radius=10,
        border=ft.border.all(1, "#86B3F2B5"),
    )

    tarjeta_carga = ft.Container(
        content=ft.Column([
            ft.Text("Carga Familiar (Miembros)", size=18, weight="bold", color=COLOR_NEGRO),
            ft.Divider(height=15),
            # Fila 1 de entrada
            ft.Row([
                nombres_miembro,
            ...
            # Fila 2 de entrada + Botón Añadir alineado
            ft.Row([
            ...
            ft.Container(
                ...
                bgcolor="#9FB0F39D",
                border_radius=8,
                border=ft.border.all(1, "#A8B5F1B7"),
            ),
        ], spacing=10),
        padding=20,
        bgcolor="#80ABE6B4",
        border_radius=10,
        border=ft.border.all(1, "#7B9AE3A0"),
    )
